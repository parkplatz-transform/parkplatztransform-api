...
def send_verification_token(email: str, token: str =""):
    ...
    return requests.post(f"https://api.mailgun.net/v3/{settings.mailgun_domain}/messages",
    auth=("api", settings.mailgun_api_key),
    data={"from": "Park Platz Verification verify@parkplatztransform.com",
          "to": [email],
...
async def send_verification(email: str, background_tasks: BackgroundTasks):
    background_tasks.add_task(send_verification_token, email, token="")
